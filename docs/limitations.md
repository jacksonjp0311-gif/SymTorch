...
The `0.12.0` WebGPU line can upload tensor data into GPU buffers, run same-shape `float32` elementwise kernels, read results back explicitly, and run a browser parity gate for add when WebGPU is available. It does not yet provide general tensor dispatch, broadcasting, autograd on GPU, or broad hardware coverage.
