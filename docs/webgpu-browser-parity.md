...
SymTorch `0.11.0` added the first browser-side WebGPU parity gate. SymTorch `0.16.0` expands it to the explicit kernel set, including scalar `sumAll`, composed `meanAll`, and stable scalar `logSumExpAll`.
...
- Uploaded CPU data can be processed by binary elementwise, unary elementwise, scalar reduction, scalar composition, and scalar stability paths.
