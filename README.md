...
SymTorch is early, active, and intentionally foundation-first. The `0.15.0` workspace line is the WebGPU mean composition line: CPU remains the correctness oracle, explicit WebGPU kernels now cover the same-shape elementwise set plus scalar `sumAll`, `meanAll` is composed from `sumAll` and scalar division, and browser parity gates the explicit kernel set when WebGPU is available.
